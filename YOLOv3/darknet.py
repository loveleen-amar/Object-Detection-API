...
class DetectionLayer(nn.Module):
    ...
    def forward(self, x, inp_dim, num_classes, confidence):
        ...
        global CUDA
        prediction = x
        prediction = util.predict_transform(prediction, inp_dim, self.anchors, num_classes, confidence, CUDA)
        return prediction
        ...
        for i in range(len(self.module_list)):
            ...
            if (module_type) == "convolutional":
                ...
                if (batch_normalize):
                    ...
                    util.convert2cpu(bn.bias.data).numpy().tofile(fp)
                    util.convert2cpu(bn.weight.data).numpy().tofile(fp)
                    util.convert2cpu(bn.running_mean).numpy().tofile(fp)
                    util.convert2cpu(bn.running_var).numpy().tofile(fp)
